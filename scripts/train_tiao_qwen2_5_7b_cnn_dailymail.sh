#!/bin/bash
#SBATCH --job-name=TIAO-Qwen2.5-7B-CD
#SBATCH --nodes=8
#SBATCH --ntasks=8
...
# TIAO training on CNN/DailyMail: 8 nodes x 4 GPUs = 32 ranks.
set -Eeo pipefail
...
RUN_OUTPUT_DIR="${TIAO_RUN_OUTPUT_DIR:-${OUTPUT_ROOT}/tiao/${RUN_ID}-${MODEL_NAME}}"
FINAL_MODEL_DIR="${TIAO_FINAL_MODEL_DIR:-${RUN_OUTPUT_DIR}/final-model}"
JOB_LOG="${TIAO_JOB_LOG:-${OUTPUT_ROOT}/tiao-training-${RUN_ID}.log}"
...
on_exit() {
  status=$?
  if [[ ${status} -eq 0 ]]; then
    echo "[$(date --iso-8601=seconds)] TIAO training completed successfully."
    echo "Final model: ${FINAL_MODEL_DIR}"
  else
    echo "[$(date --iso-8601=seconds)] TIAO training failed with exit code ${status}." >&2
...
echo "============================================================"
echo "TIAO formal training on CNN/DailyMail"
...
srun \
  ...
  /bin/bash -c '
    ...
    while [[ ! -r "${REPOSITORY_ROOT}/tiao.py" \
