#!/usr/bin/env python3
"""Train TIAO with Qwen2.5-7B-Instruct on CNN/DailyMail."""
...
from tiao_trainer import TIAOConfig, TIAOTrainer
...
DEFAULT_UNIEVAL_MODEL = "MingZhong/unieval-sum"


@dataclass
class ModelArguments:
    """Model, evaluator, dataset, and final-artifact arguments."""

    base_model_name_or_path: str = field(
        default=DEFAULT_MODEL,
        metadata={"help": "Local path or model ID for Qwen2.5-7B-Instruct."},
    )
    unieval_model_name_or_path: str = field(
        default=DEFAULT_UNIEVAL_MODEL,
        metadata={"help": "Local path or model ID for the UniEval model."},
    )
    dataset_path: str = field(
        default=DEFAULT_DATASET,
        metadata={
            "help": (
                "A local CNN/DailyMail 3.0.0 Parquet directory or the "
                "cnn_dailymail dataset ID."
            )
        },
    )
    final_model_output_dir: Optional[str] = field(
        default=None,
        metadata={"help": "Directory for the loadable model saved after training."},
    )
    unieval_model_deepspeed_config: Optional[str] = field(
        default=None,
        metadata={"help": "Optional DeepSpeed inference configuration for UniEval."},
    )
    unieval_max_length: int = field(
        default=1024,
        metadata={"help": "Maximum UniEval input length."},
    )
    dataset_sample_train_num: Optional[int] = field(
        default=None,
        metadata={"help": "Random training subset size; unset uses the full split."},
    )
    dataset_sample_eval_num: Optional[int] = field(
        default=None,
        metadata={"help": "Random validation subset size; unset uses the full split."},
    )
    random_seed: int = field(
        default=1999,
        metadata={"help": "Seed used for data sampling and training."},
    )
...
def _local_or_remote(identifier: str) -> bool:
    """Return the local_files_only flag expected by Transformers."""
...
def _load_cnn_dailymail_split(
    dataset_path: str,
    split: str,
    sample_count: int | None,
    seed: int,
) -> Dataset:
    """Load and format one CNN/DailyMail 3.0.0 split."""

    if split not in {"train", "validation"}:
        raise ValueError("TIAO training accepts only train and validation splits")

    dataset_directory = Path(dataset_path)
    if dataset_directory.is_dir():
        data_files = {
            "train": str(dataset_directory / "train-*.parquet"),
            "validation": str(dataset_directory / "validation-*.parquet"),
        }
        raw_dataset = load_dataset("parquet", data_files=data_files, split=split)
    elif dataset_path == DEFAULT_DATASET:
        raw_dataset = load_dataset(DEFAULT_DATASET, "3.0.0", split=split)
    else:
        raise FileNotFoundError(
            "dataset_path must be a CNN/DailyMail Parquet directory or "
            f"'{DEFAULT_DATASET}'"
        )

    if sample_count is not None:
        if sample_count <= 0:
            raise ValueError("Dataset sample counts must be positive when provided")
        raw_dataset = raw_dataset.shuffle(seed=seed).select(
            range(min(sample_count, len(raw_dataset)))
        )

    def format_example(example: dict[str, str]) -> dict[str, object]:
        article = str(example["article"])
        return {
            "prompt": [{"role": "user", "content": _build_prompt(article)}],
            "origin_text": article,
            "ref_text": str(example["highlights"]),
        }
    ...
    return raw_dataset.map(
        ...
        keep_in_memory=True,
        desc=f"Formatting CNN/DailyMail {split}",
    )


def _completion_text(completion: object) -> str:
    """Extract text from TRL's conversational completion representation."""
    ...
    raise TypeError("Unexpected completion representation")


def main() -> None:
    parser = HfArgumentParser((TIAOConfig, ModelArguments))
    ...
    final_model_directory = Path(
        ...
        or REPOSITORY_ROOT / "outputs" / "tiao" / model_name / "final-model"
    )
    Path(training_args.output_dir).mkdir(parents=True, exist_ok=True)

    if is_main:
        print("=" * 72)
        print("TIAO training configuration")
        print("=" * 72)
        print(training_args)
    ...
    if evaluator_deepspeed and evaluator_deepspeed.lower() in {"none", "null", "false"}:
